// G_synthetic/components/NodeDetail.tsx
...
export const NodeDetail: React.FC<NodeDetailProps> = ({ node, registry, onClose }) => {
  ...
  const resolve = (face: FaceType) => {
    ...
    // FIX: Prevent React crash by stringifying unexpected objects
  ...
  return (
    <div className="absolute top-0 right-0 w-80 h-full bg-neutral-900/95 border-l border-cyan-900/50 p-6 backdrop-blur-md z-50 transform transition-transform duration-300 overflow-y-auto shadow-2xl">
      <div className="flex justify-between items-center mb-6 border-b border-cyan-800 pb-2">
        <h3 className="text-cyan-400 font-bold text-lg tracking-widest">NODE_{node.x}{node.y}{node.z}</h3>
      ...
      <div className="space-y-6 font-mono text-sm">
        {Object.values(FaceType).map((face) => (
          <div key={face} className="group">
            <h4 className="text-xs uppercase text-neutral-500 mb-1 flex items-center gap-2">
            ...
            <div className="p-3 bg-black/40 border border-white/5 rounded text-neutral-300 break-words group-hover:border-cyan-500/30 transition-colors">
              {resolve(face) || <span className="italic text-neutral-600">Null</span>}
            </div>
          </div>
        ))}
      </div>
      
      <div className="mt-8 text-[10px] text-neutral-600 text-center font-mono">
